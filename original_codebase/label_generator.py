...
just defining labels for digits 0-9
the labels are rectanges centred around the 10 given points
...
def eval_accuracy(target, label):
    ...
    label_hat = torch.zeros(target.size(0), 10)
    ...
    acc = (label_hat == label).sum()/label.size(0)
