...
def train(onn,
    ...
    for epoch in range(epoch_num):  
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            trained_outputs = model(inputs)
            I_ = (trained_outputs * trained_outputs.conj()).real
            outputs = onn(inputs)
            I = (outputs * outputs.conj()).real
            loss = criterion(I, I_)

            #upstream gradient: dL/dO
            dL_dw1, = torch.autograd.grad(loss, model.parameter, retain_graph=True)

            # assign custom gradient
            onn.parameter.grad = -dL_dw1
...
if __name__ == '__main__':
    ...
    tilt = torch.tensor([[0.1,0.1],[0.1,0.1]])
    ...
    parameter = model.parameter.detach().clone()
    ...
    onn = Onn(M, L, lambda0, parameter, z, d, tilt).to(device)
    onn = onn.to(device)
    epoch_num = 30
    ...
    onn, train_losses, train_accies, val_losses, val_accies, I_val, labels_val =\
            train(onn, model, criterion, optimizer,
